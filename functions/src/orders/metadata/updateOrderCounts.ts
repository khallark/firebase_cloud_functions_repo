...
const updateBlockedStock = async (
  ...
  afterStatus: InventoryStatus,
  line_items: any[],
  beforeCustomStatus: string | null, // ← add this
  afterCustomStatus: string | null, // ← add this
) => {
  if (beforeStatus === afterStatus) {
    console.log(`⏭️ No inventory status change (${beforeStatus}), skipping blocked stock update`);
    return;
  ...
  // UPC flow handles this atomically — skip to avoid double decrement
  if (beforeCustomStatus === "Ready To Dispatch" && afterCustomStatus === "Dispatched") {
    console.log(`⏭️ Skipping blockedStock update — handled by onUpcWritten`);
    return;
  }
