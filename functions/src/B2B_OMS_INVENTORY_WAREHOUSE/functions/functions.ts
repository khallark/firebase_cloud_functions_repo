  ...
  async (req, res) => {
    ...
    try {
      ...
      if (existingDoc.exists) {
        res.status(400).json({ error: "sku_already_exists", message: `A product with SKU "${productId}" already exists.` });
        return;
      }
...
export const createRawMaterial = onRequest(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (existingDoc.exists) {
        res.status(400).json({ error: "sku_already_exists", message: `A raw material with SKU "${materialId}" already exists.` });
        return;
      }
