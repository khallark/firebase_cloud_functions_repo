...
export const processPropagationTask = onRequest(
    ...
    async (req, res) => {
        try {
            ...
            // Parse task payload
            const task: PropagationTask = JSON.parse(
                Buffer.from(req.body, 'base64').toString()
            );

            console.log(`Processing ${task.type} chunk ${task.chunkIndex}/${task.totalChunks} for ${task.entityId}`);
...
async function processShelfLocationChunk(task: PropagationTask) {
    const { businessId, entityId: shelfId, data, chunkIndex, chunkSize, propagationId, version } = task;
...
async function processRackLocationChunk(task: PropagationTask) {
    ...
    const [rackId, collection] = task.entityId.split('-');
...
async function processZoneLocationChunk(task: PropagationTask) {
    ...
    const [zoneId, collection] = task.entityId.split('-');
    const trackerRef = db.doc(`users/${businessId}/propagation_trackers/${propagationId}`);
